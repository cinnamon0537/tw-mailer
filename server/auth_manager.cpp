...
AuthManager::AuthManager(const std::string& blacklistFile) 
  : blacklistFile_(blacklistFile), attemptsFile_(blacklistFile + ".attempts") {
...
std::string AuthManager::makeKey(const std::string& ip, const std::string& username) {
  return ip + ":" + username;
}
...
bool AuthManager::recordFailedAttempt(const std::string& ip, const std::string& username) {
  std::string lockfile = blacklistFile_ + ".lock";
  int lockfd = acquireLock(lockfile);
...
void AuthManager::recordSuccess(const std::string& ip, const std::string& username) {
  std::string lockfile = blacklistFile_ + ".lock";
  int lockfd = acquireLock(lockfile);
  if (lockfd < 0) return;
...
void AuthManager::cleanupExpiredEntries(std::map<std::string, BlacklistEntry>& blacklist,
                                       std::map<std::string, AttemptInfo>& attempts) {
  std::time_t now = std::time(nullptr);
...
void AuthManager::loadBlacklist(std::map<std::string, BlacklistEntry>& blacklist) {
...
void AuthManager::saveBlacklist(const std::map<std::string, BlacklistEntry>& blacklist) {
...
void AuthManager::saveAttempts(const std::map<std::string, AttemptInfo>& attempts) {
...
bool AuthManager::authenticateLDAP(const std::string& username, const std::string& password) {
    ...
    std::string uri = std::string("ldap://") + LDAP_HOST + ":" + std::to_string(LDAP_SERVER_PORT);

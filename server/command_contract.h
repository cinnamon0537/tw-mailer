...
struct Context {
  ...
  std::string authenticatedUser;  // Empty if not authenticated, username if
                                  // authenticated
  std::string clientIP;  // Client IP address for rate limiting and blacklisting
};
...
struct CommandOutcome {
  ...
  std::string
      response;  // length-prefixed Antwort (OK/ERR/…); leer = keine Antwort
};
...
class ICommand {
 public:
  ...
  virtual CommandOutcome execute(Context& ctx,
